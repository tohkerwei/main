...
. Pressing the kbd:[Up] arrow key will browse backwards towards the least recently entered commands
. Pressing the kbd:[Down] arrow key will browse forwards towards the most recently entered commands
...
Similar to the previously mentioned "Command History" feature, this feature improves the user experience by allowing users to press the kbd:[Tab] key to auto complete their partially entered commands.

==== Implementation

This feature is mostly facilitated by the logic found in the `AutoComplete` class. Firstly, let us define the definitions of _unambiguous_ and _ambiguous_ commands. Unambiguous commands are those that can be uniquely identified by the sequence of letters that the user has entered. Ambiguous commands are those that cannot be identified by the said sequence of letters. For example, assume we only have 3 commands in our app, `add-c`, `add-e`, and `edit-c`. If the user enters `e` and presses kbd:[Tab], we say that this is an unambiguous command since clearly only `edit-c` can be the correct command. If instead, the user enters `a` and presses kbd:[Tab], we say that this is an ambiguous command, since both `add-c` and `add-e` are possible choices. Again, let us start by defining the behaviour (which also has been implemented to mimic most modern CLIs):
...
** ie. using the example in the introduction above, when the user enters `a` and presses kbd:[Tab], the autocompletion should be till `add-`
. A list of all similar commands should be presented to the user should he try to autocomplete an ambiguous command

A limitation of the current design is that we are only able to autocomplete commands, and not other fields like names and addresses that have been used before. However, since all commands defined in FitBiz do not have empty spaces in them, this allows us to easily determine when to disable this feature. As such the following code snippet has been used in `AutoComplete#execute` to stop execution of the code if white spaces has been detected:
...
To implement this feature, we have decided to use a Trie data structure. Since no such data structure exists natively in Java, we had to implement our own version of it. Of course, we also considered other much simpler alternatives like simply storing all available commands in a `List`. A quick summary is given here:
