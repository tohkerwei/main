...
A limitation of the current design is that we are only able to autocomplete commands, and not other fields/parameters like names and addresses that have been used by the user before. However, since all commands defined in FitBiz do not have empty spaces in them, this allows us to easily determine when to disable this feature. As such the following code snippet has been used in `AutoComplete#execute` to stop execution of the code if white spaces has been detected:
