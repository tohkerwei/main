...
command and store any possible changes in an `EditClientDescriptor`. Recall that we can verify the contents of
`editClientDesciptor` through the `Variable` tool window.
+
...
----
. It appears that a `JsonAdaptedClient` is created for each `Client` and then added to the
