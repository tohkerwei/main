...
In scenarios like this, you can consider refactoring the existing `Client` model to suit your use case.
...
The `address` field in `Client` is actually an instance of the `seedu.address.model.client.Address` class.
...
Other usages like in `EditClientDescriptor` may require more careful inspection.

Let's try removing references to `Address` in `EditClientDescriptor`.

. Safe delete the field `address` in `EditClientDescriptor`
...
For example, removing the `address` field from the `Client` class will require you to modify its constructor.
...
For example, there is a `$address` in each `ClientCard` that has not been removed nor identified.
